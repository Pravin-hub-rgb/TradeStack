...
def test_update_cache_for_date():
    ...
    assert result["total_cached_stocks"] >= 1
    assert result["bhavcopy_stocks"] > 500
    ...
    print(f"{PASS} test_update_cache_for_date ({used_date}: {result['updated']} updated in {result['duration_sec']}s)")
    return result
...
def test_update_idempotent():
    ...
    assert result2["skipped"] >= result2["total_cached_stocks"]
