...
const Lightfall: React.FC<LightfallProps> = ({
  ...
  dpr,
  paused = false,
  colors: _colors = ['#A6C8FF', '#5227FF', '#FF9FFC'],
  backgroundColor: _bgColor = '#0A29FF',
  speed = 0.5,
  streakCount = 2,
  streakWidth = 1,
  streakLength = 1,
  ...
  const colors = useMemo(() => _colors, [_colors.join('')]);
  const backgroundColor = useMemo(() => _bgColor, [_bgColor]);
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uMouseColor: { value: mouseColor ? hexToRGB(mouseColor) : avg },
      uSpeed: { value: speed },
