...
def test_ohlc_opening_price_final_issue():
    ...
    try:
        ...
        # Get OHLC data with interval=1d for day's session
        url = f"https://api.upstox.com/v3/market-quote/ohlc?instrument_key={instrument_key}&interval=1d"
        headers = {
        ...
        if response.status_code == 200:
            ...
            if response_data.get('status') == 'success':
                ...
                live_open = live_ohlc.get('open') if live_ohlc else None
                prev_close = prev_ohlc.get('close') if prev_ohlc else None
                ...
                if live_open:
                    ...
                    if live_open == ltp_data.get('ltp'):
                        print("  WARNING: live_ohlc.open matches current LTP - API returns wrong data")
                    else:
                        print("  CONFIRMED: live_ohlc.open is different from current LTP - API returns correct data")
                else:
